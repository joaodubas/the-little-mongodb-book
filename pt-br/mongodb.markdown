...
## Iniciando ##
A maior parte deste livro focará nas funcionalidades *core* do MongoDB. Sendo assim, iremos utilizamr o shell do MongoDB. Mesmo o shell sendo útil para aprendizado, bem como, para atividades administrativas, seu código usará um driver do MongoDB.

Isso trás a tona a primeira coisa que você deve saber sobre MongoDB: seus drivers. MongoDB tem um [número oficial de drivers](http://www.mongodb.org/display/DOCS/Drivers) para várias linguagens. Esses drivers podem ser entendidos como os diversos drivers de banco de dados aos quais você já deve estar familiarizado. Em cima destes drivers, a comunidade de desenvolvedores criou mais bibliotecas para outras linguagens/frameworks. Por exemplo, [NoRM](https://github.com/atheken/NoRM) é uma biblioteca C# quem implementa LINQ e [MongoMapper](https://github.com/jnunemaker/mongomapper) é uma biblioteca Ruby que é similar ao ActiveRecord. Pode-se programar diretamente com os drivers do MongoDB ou alguma biblioteca de maior nível, esta é uma decisão sua. Aponto isso somente porque muitas pessoas novas ao MongoDB ficam confusas em relação à existencia tanto de drivers oficiais e bibliotecas da comunidade: o primeiro geralmente foca na comunicação/conectividade com o MongoDB, por sua vez, o segundo tem ênfase em implementações específicas da linguagem ou framework.

Conforme seguimos, encorajo você a utilizar o MongoDB para replicar o que foi demonstrado, bem como explorar questões que possam surgir naturalmente. É bastante simples instalar o MongoDB, vamos tomar alguns minutos para preparar tudo.

1. Vá para a [página oficial de download](http://www.mongodb.org/downloads) e pegue os binários da primeira linha (a versão estável recomendada) para o sistema operacional de sua escolha. Para propósitos de desenvolvimento, você pode baixar tanto 32-bit quanto 64-bit.

2. Extraia o arquivo (onde você quiser) e navegue até a pasta `bin`. Não execute nada ainda, mas saibe que `mongod` é o processo do servidor e `mongo` é o shell do cliente - estes são os dois executáveis nos quais passaremos a maior parte do tempo.

3. Crie um novo arquivo de texto na pasta `bin` chamado `mongodb.config`

4. Adicione uma linha no mongodb.config: `dbpath=CAMINHO_PARA_ONDE_VOCÊ_DESEJA_ARMAZENAR_SEUS_ARQUIVOS_DO_BANCO_DE_DADOS`. Por exemplo, no Windows você pode fazer `dbpath=c:\mongodb\data` e no Linux é possível fazer `dbpath=/etc/mongodb/data`.

5. Tenha certeza de que o `dbpath` que você específicou existe.

6. Execute mongod com o parâmetro `--config /caminho/para/seu/mongodb.config`.

Como um exemplo para os usuários de Windows, se você extraiu o arquivo baixado em `c:\mongodb\` e criou `c:\mongodb\data\` então dentro de `c:\mongodb\bin\mongodb.config` você deverã especificar `dbpath=c:\mongodb\data\`. Então você pode executar `mongod` a partir do prompt de comando via `c:\mongodb\bin\mongod --config c:\mongodb\bin\mongodb.config`.

Sinta-se a vontade para adicionar a pasta `bin` ao seu *path* para tornar os comandos mais curtos. Usuários do MacOSX e Linux podem seguir instruções parecidas. A única coisa que vocês devem alterar são os caminhos.

Com sucesso, você deve ter um MongoDB pronto para ser utilizado. Caso você receba uma mensagem de erro, leia o console cuidadosamente - o servidor é muito bom ao explicar o que houve de errado.

Você pode agorar executar `mongo` (sem o *d*) que conectará um shell à sua instância do servidor. Digite `db.version()` para assegurar-se de que tudo esta funcionando como deveria. Caso tudo esteja correto, você verá o número da versão instalada.
